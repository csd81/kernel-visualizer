...
function FrameGridInner({ frames, onSelect, flash }: Props) {
  return (
    <div className={`grid grid-cols-16 gap-[2px] ${flash ? "animate-page-fault" : ""}`}>
      {frames.map(f => (
        <button
          key={f.id}
          onClick={() => onSelect(f)}
          className="aspect-square rounded-sm text-[5px] lg:text-[6px] font-mono transition-all duration-200 hover:brightness-150 hover:scale-110 cursor-pointer border-0 min-w-[16px] min-h-[16px] lg:min-w-[20px] lg:min-h-[20px]"
          style={{
